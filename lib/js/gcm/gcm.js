    ...
    if (gcm.change_manager.has_changes()) {
        ...
        gcm.change_manager.walk_changeset(function (id, change) {
            ...
            switch (change.type) {
                case 'cp_mod':
                case 'attr_mod':
                    apply_attribute_change(id, change, character);
                    break;

                case 'remove':
                    remove_by_path(id, change, character);
                    break;
...
function remove_by_path(id, change, character) {
}

function apply_attribute_change(id, change, character) {
    var obj_path = change.key.split('.');
    var target_obj = character;

    gcm.log('Processing change: ' + id + '(' + JSON.stringify(change) + ')');

    for (var i in obj_path) {
        if (is_not_set(target_obj)) {
            gcm.log('Unable to locate object by path ' + change.key);
            break;
        }
        ...
        if (i + 1 < obj_path.length) {
            ...
            target_obj[path_part] = change.value;
        }
    }
}
