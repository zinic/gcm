...
window.gcm = (function () {
    return {
        ...
        libraries: [
            '/lib/js/gcm/common.lib.js',
            '/lib/js/gcm/json.lib.js',
            '/lib/js/gcm/gurps.lib.js',
            '/lib/js/gcm/html5_localstore.lib.js',
            '/lib/js/gcm/changes.lib.js'
        ],
...
function gcm_init() {
    ...
    gcm_load_libraries(info_element);
    gcm_load_templates(info_element);

    info_element.html('<p>Loading changes</p>');
    gcm.change_manager.load();

    gcm_load_character('./lib/example_character.json', info_element);

    info_element.addClass('hidden');
}

function gcm_load_libraries(info_element) {
    for (var i in gcm.libraries) {
        var lib_path = gcm.libraries[i];

        if (is_set(info_element)) {
            info_element.html('<p>Loading library: ' + lib_path + '</p>');
        }

        gcm.include(lib_path);
    }
};
...
function gcm_save() {
    gcm.change_manager.trim_at_bookmark();
    gcm.change_manager.save();
}
...
function gcm_remove_skill(name) {
...
function gcm_render_character_attributes(attributes) {
    ...
    $('#basic_move_total_value').html(gcm.gurps.basic_move(attributes));
    $('#basic_speed_total_value').html(gcm.gurps.basic_speed(attributes));
