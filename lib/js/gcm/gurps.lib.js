...
var gurps = {
    ...
    skill_cp_cost: function (skill) {
        var cost = gurps.SKILL_DIFFICULTY_CP_COSTS[skill['difficulty']];
    ...
    basic_move: function (attributes) {
        return gurps.basic_speed(attributes) + attributes['basic_move'];
    },
    ...
    language_cost: function (language) {
        var fluency_modifier = gurps.LANGUAGE_FLUENCY_COST_MAP[language['fluency']];
        var cost = 0;
    ...
    culture_features_cp_cost: function (culture_features) {
        ...
        for (var i in languages) {
            cp_cost += gurps.language_cost(languages[i]);
        }
    ...
    skills_cp_cost: function (skills) {
        ...
        for (var i in skills) {
            ...
            if (!skill['disabled']) {
                var cost = gurps.skill_cp_cost(skills[i]);
