...
function gcm_clean_editable_elements() {
    ...
    for (var attr_key in attributes) {
        ...
        gcm_log('Attempting to destroy "' + target_element + '" editable.');

        $(target_element).editable('destroy')
    }
}

function gcm_init_editable_elements() {
    var attributes = gcm.character.attributes;
            ...
            success: (
                function (name) {
                    return function (response, new_value) {
                        gcm.character.attributes[name] = parseFloat(new_value);
    ...
    $('#initial_cp_value').editable({
        ...
        success: function (response, new_value) {
            gcm.character.initial_cp = parseInt(new_value);
            gcm_render_character();
        }
    });
    ...
    $('#bonus_cp_value').editable({
        ...
        success: function (response, new_value) {
            gcm.character.bonus_cp = parseInt(new_value);
            gcm_render_character();
        }
    });
}
