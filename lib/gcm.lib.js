            ...
            'success': (
                function (name) {
                    return function (response, new_attr_value) {
                        gcm_log('new value for ' + name + ': ' + new_attr_value);

                        gcm.character.attributes[name] = new_attr_value;
...
function gcm_render_character_movement(attributes) {
    ...
    var movement_obj = {
        ...
        "dodge": {
            "none": Math.floor(basic_speed),
            "light": Math.floor(basic_speed) - 1,
            "medium": Math.floor(basic_speed) - 2,
            "heavy": Math.floor(basic_speed) - 3,
            "x_heavy": Math.floor(basic_speed) - 4
        }
    };
